...
void DelNotNiceStudents(Students Group) {
    ...
    if (nice_index == (size_t*)NULL) {
        exit(EXIT_FAILURE);
    }

    del_index = (size_t*)malloc(sizeof(size_t));
    if (del_index == (size_t*)NULL) {
        exit(EXIT_FAILURE);
    }
    ...
    for (i = 0; i < Group->count; ++i) {
        if ((double)NICE_MARKS > (double)3) {
            tmp_index =
                (size_t*)realloc(nice_index, (nice_count + 1) * sizeof(size_t));
            if (tmp_index == (size_t*)NULL) {
                exit(EXIT_FAILURE);
            }
            ...
            tmp_index =
                (size_t*)realloc(del_index, (del_count + 1) * sizeof(size_t));
            if (tmp_index == (size_t*)NULL) {
                exit(EXIT_FAILURE);
            }
